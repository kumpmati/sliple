...
import { db } from '..';
import { calculatePercentile } from '$lib/v2/stats/percentile';
import { fill } from '$lib/v2/stats/fill';

export const getPuzzleStatistics = async (
	puzzleId: string,
	numMoves: number
): Promise<V2Statistics> => {
	const [totals, distribution] = await Promise.all([
		db
			.select({
				averageMoves: sql<number>`cast(avg(${puzzleCompletionTable.numMoves}) as int)`,
	...
	return {
		percentile: calculatePercentile(filledDistribution, numMoves),
